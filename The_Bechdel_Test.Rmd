...
It is the pipe tool and it feeds in data to a function.
...
it gives you the summary statistics for the variable that you call with it. along with this it gives you the statistic that you call for. 
...
it lets you select a variable such as title and then it brings the data that is in that column. this is good for when you want to look at a specific variable or column.
...
they are paranomral activity, the Blair witch project and El mariachi. we can see if there are more by filtering out the data and seeing their names.  
...
ggplot(data = bechdel90_13, mapping = aes(x = clean_test, y = roi, color = binary)) +
  geom_boxplot() +
  labs(title = "Return on investment vs. Bechdel test result",
       subtitle = "Only movies with RoI's of < 400", # Something about zooming in to a certain level
       x = "Detailed Bechdel result",
       y = "Return on investment",
       color = "Binary Bechdel result") +
...
what i learned is how to manioulate and look at data while beign able to remove certain points of the data. i also learned that therer was better ways than to make a new data set with the removed data point. I also leanred that you can make filtering easier by working with values. along with this i learned about the summarise() and the select() functions. 
